...
Reducer.prototype.addSetter = function (key) {
  this.subscribe(['SIMR', 'SET', this.name, key].join('.'), function(state, action) {
...
Reducer.prototype.addConcatTo = function (key) {
  this.subscribe(['SIMR', 'CONCAT', this.name, key].join('.'), function(state, action) {
    state[key] = state[key].concat(action.value);
    return state;
  });
};
Reducer.prototype.addUpdateIndex = function (key) {
  this.subscribe(['SIMR', 'UPDATE_INDEX', this.name, key].join('.'), function(state, action) {
    state[key][action.index] = action.updateFn(state[key][action.index]);
    return state;
  });
};
Reducer.prototype.addUpdateID = function (key) {
  this.subscribe(['SIMR', 'UPDATE_ID', this.name, key].join('.'), function(state, action) {
